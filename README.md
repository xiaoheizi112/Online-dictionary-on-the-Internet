...
## 实现功能
- 搭建基于Socket的TCP/IP通信架构，实现客户端-服务器模型，支持用户注册、登录、单词查询和历史记录查看等完整功能链。
- 采用SQLite3构建轻量级数据库，存储用户账号信息和查询历史记录，确保数据持久化和安全性。
- 应用Epoll技术实现IO多路复用，高效处理多客户端并发连接请求，显著提升系统响应速度和并发性能。
